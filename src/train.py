...
class Cifar10Model(Trainable):
    ...
    def setup(self, config):
        ...
        opt = tf.keras.optimizers.Adadelta(
            learning_rate=self.config.get("lr", 1e-4), weight_decay=self.config.get("decay", 1e-4)
        )
        model.compile(
    ...
    def step(self):
        ...
        self.model.fit(gen, epochs=self.config.get("epochs", 1), validation_data=None)
...
if __name__ == "__main__":
    ...
    space = {
        "epochs": 1,
        "batch_size": 64,
        "lr": 10 ** -2,  # tune.grid_search([10**-2]),
        "decay": tune.sample_from(lambda spec: spec.config.lr / 100.0),
        "dropout": 0.5  # tune.grid_search([0.5]),
    }
    ...
    best_model = load_model(best_checkpoint.path + "/model.keras")

    # Save the best model to a desired path
    model_path = "models/teacher_model.keras"
